...
### Convention des message commit

Les messages pour les commit sont essentiels afin de pouvoir revenir en arrière si besoin. Pour cette raison ils doivent être clairs et anglais. La convention que nous suivrons est la suivante :

```jsx
<TYPE>(<file_or_folder_or_feature>) <detail>
```

Le type du commit est écrit en capital est doit être une des valeurs suivantes :

* `ADD`: Ajout d'un module, dossier, asset...
* `CHORE`: Changement dans la configuration/set-up
* `DEL`: Suppression d'un module, dossier, asset
* `FEAT`: Ajout/implémentation d'un feature
* `FIX` : réparation de bug
* `REM`: Suppression d'un feature 
* `REFACTOR`: Réusinage de code ne changeant le comportement du code
* `STYLE`: Changement de style, nettoyage de code
* `TEST`: implémentation de tests

***Exemples de message de  commit:***

```bash
git commit -m 'ADD(notes/assets/friends-stagingarea.gif)'
git commit -m 'CHORE(.gitignore) add .venv'
git commit -m 'DEL(core/db.py)'
git commit -m 'FEAT(parser) add email extraction'
git commit -m 'FIX(login) load .env'
git commit -m 'REM(db/create_url) use built-in sqlalchemy function'
git commit -m 'REFACTORE(db_connection) use session instead of engine connection'
git commit -m 'STYLE(models.py) clean up unused table classes'
git commit -m 'TEST(utils/mean.py) add unit test'
```
...
git add feature.py
# commit changes
git commit -m 'FIX function parse'
